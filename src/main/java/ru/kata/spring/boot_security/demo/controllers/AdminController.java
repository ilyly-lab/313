...
public class AdminController {
    ...
    @GetMapping("")
    public String pageForCrudOperationsAdmin(Principal principal, Model model) {
    ...
    public String postAddUser(@ModelAttribute("user") @Valid User user,
                              ...
                              @RequestParam(required=false) String roleUser, BindingResult bindingResult, Model model) {
        userValidator.validate(user, bindingResult);
        if (bindingResult.hasErrors()) {
            //model.addAttribute("activeTab", "nav-profile-tab");
            return "redirect:/admin";
        }
        List<Role> roles = new LinkedList<>();
        if (roleAdmin != null && roleAdmin.equals("ROLE_ADMIN")) {
